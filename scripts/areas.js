...
btn_cancel.click((e) => {
   e.preventDefault();
   modal_title.html("<i class='fa-regular fa-circle-plus'></i>&nbsp; REGISTRAR ÁREA");
...
async function fillTable(show_toas = true) {
   ...
   objResponse.map((obj) => {
      ...
      list.push([column_area, column_buttons]);
   });
   //Dibujar Tabla
...
async function deleteObj(btn_delete) {
   ...
   ajaxRequestQuestionAsync(title, text, URL_AREA_APP, data, "fillTable()");
}
