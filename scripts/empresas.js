...
async function fillTable() {
   ...
   objResponse.map((obj) => {
      //Campos
      let column_logo =
            obj.logo_path == "" || obj.logo_path == null
               ? `<img class="img-fluid rounded-lg" src="../assets/img/cargar_imagen.png" style="max-height: 150px !important;" />`
               : `<img class="img-fluid rounded-lg" src="../assets/img/${obj.logo_path}" style="max-height: 150px !important;" />`,
         column_company = `
				<b>${obj.company}</b><br>
...
async function editObj(btn_edit) {
   ...
   if (obj.logo_path == "" || obj.logo_path == null) resetImgPreview($(`#${input_logo_path.attr("data-preview")}`));
      ...
      resetImgPreview($(`#${input_logo_path.attr("data-preview")}`), `/empleos/assets/img/${obj.logo_path}`);
