...
btn_cancel.click((e) => {
   e.preventDefault();
   modal_title.html("<i class='fa-regular fa-circle-plus'></i>&nbsp; REGISTRAR ETIQUETA");
...
async function fillTable(show_toas = true) {
   ...
   objResponse.map((obj) => {
      ...
      if (permission_update) {
         column_buttons +=
            //html
            `<button class='btn btn-outline-primary btn_edit' type='button' data-id='${obj.id}' title='Editar Etiqueta'><i class='fa-regular fa-pen-to-square fa-lg i_edit'></i></button>`;
      }
      ...
      list.push([column_tag, column_buttons]);
   });
   //Dibujar Tabla
...
async function deleteObj(btn_delete) {
   ...
   ajaxRequestQuestionAsync(title, text, URL_TAG_APP, data, "fillTable()");
}
