...
btn_cancel.click((e) => {
   e.preventDefault();
   modal_title.html("<i class='fa-regular fa-circle-plus'></i>&nbsp; REGISTRAR GIRO");
...
async function fillTable(show_toas = true) {
   ...
   objResponse.map((obj) => {
      //Campos
      let column_business_line = `${obj.business_line}`;
      ...
      if (permission_update) {
         column_buttons +=
            //html
            `<button class='btn btn-outline-primary btn_edit' type='button' data-id='${obj.id}' title='Editar Giro'><i class='fa-regular fa-pen-to-square fa-lg i_edit'></i></button>`;
      }
      ...
      list.push([column_business_line, column_buttons]);
   });
   //Dibujar Tabla
...
async function deleteObj(btn_delete) {
   ...
   ajaxRequestQuestionAsync(title, text, URL_BUSINESS_LINE_APP, data, "fillTable()");
}
