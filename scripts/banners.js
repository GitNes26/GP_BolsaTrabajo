...
$(document).ready(() => {
   ...
   function actualizarStatusRegistros() {
      ...
      if (por_actualizar) {
         peticionAjax(url_modelo_app, datos, cambioDeEstado, null);
      }
   }
   function cambioDeEstado(ajaxResponse) {
      ...
      tds_active.forEach((td_active) => {
         ...
         if (objResponse.includes(id_registro)) {
            ...
            if (!td_order_view.hasClass("text-muted")) {
               td_order_view.addClass("text-muted");
            }
...
btn_cancel.click((e) => {
   e.preventDefault();
   modal_title.html("<i class='fa-regular fa-circle-plus'></i>&nbsp; REGISTRAR BANNER");
...
input_active.click(() => {
   ...
   if (estado) {
      label_input_active.text("Activo");
      input_active.attr("data-activo", 1);
   } else {
      label_input_active.text("Inactivo");
      input_active.attr("data-activo", 0);
   }
});
...
async function fillTable(show_toas = true) {
   ...
   objResponse.map((obj) => {
      ...
      if (obj.order_view == 1000000) {
         class_handle = "text-muted";
         obj.order_view = "";
      }
      ...
      //Campos
      let column_date_init = `${formatDatetime(obj.date_init)}`,
      ...
      list.push([column_date_init, column_date_end, column_link, column_file_path, column_order_view, column_active, column_buttons]);
   });
   //Dibujar Tabla
...
async function deleteObj(btn_delete) {
   ...
   ajaxRequestQuestionAsync(title, text, URL_BANNER_APP, data, "fillTable()");
}
