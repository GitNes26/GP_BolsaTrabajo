...
btn_cancel.click((e) => {
   e.preventDefault();
   modal_title.html("<i class='fa-regular fa-circle-plus'></i>&nbsp; REGISTRAR NIVEL DE ESTUDIO");
...
form.on("submit", async (e) => {
   ...
   // return console.log(data);
   const ajaxResponse = await ajaxRequestAsync(URL_LEVEL_APP, data);
...
async function fillTable(show_toas = true) {
   let data = { op: "index" };
   const ajaxResponse = await ajaxRequestAsync(URL_LEVEL_APP, data, null, true, show_toas);
   ...
   objResponse.map((obj) => {
      ...
      if (permission_update) {
         column_buttons +=
            //html
            `<button class='btn btn-outline-primary btn_edit' type='button' data-id='${obj.id}' title='Editar Nivel de Estudio'><i class='fa-regular fa-pen-to-square fa-lg i_edit'></i></button>`;
      }
      if (permission_delete) {
         column_buttons +=
            //html
            `<button class='btn btn-outline-danger btn_delete' type='button' data-id='${obj.id}' title='Eliminar Nivel de Estudio' data-name='${obj.level}'><i class='fa-solid fa-trash-alt i_delete'></i></button>`;
      }
      column_buttons += `</div>
					</td>`;

      list.push([column_level, column_buttons]);
   });
   //Dibujar Tabla
...
async function editObj(btn_edit) {
   modal_title.html("<i class='fa-light fa-pen-to-square'></i>&nbsp; EDITAR NIVEL DE ESTUDIO");
...
async function deleteObj(btn_delete) {
   let title = `¿Estas seguro de eliminar la nivel de estudio <br> ${btn_delete.attr("data-name")}?`;
   let text = ``;
   ...
   ajaxRequestQuestionAsync(title, text, URL_LEVEL_APP, data, "fillTable()");
}
